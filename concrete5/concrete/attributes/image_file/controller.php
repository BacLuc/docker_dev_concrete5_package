...
{
    protected $searchIndexFieldDefinition = array('type'    => 'integer',
    ...
    public function form()
    {
        ...
        $form .= $al->file('ccm-file-akID-' . $this->attributeKey->getAttributeKeyID(), $this->field('value'),
            t('Choose File'), $bf);
        $form .= '</div>';
        print $form;
    }
    ...
    public function saveValue($obj)
    {
        ...
        if (is_object($obj) && (!$obj->isError())) {
            $db->Replace('atFile', array('avID' => $this->getAttributeValueID(), 'fID' => $obj->getFileID()), 'avID',
                true);
        }
    }
