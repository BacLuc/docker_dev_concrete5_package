...
<div class="ccm-search-block-tag-cloud-wrapper ">
    ...
    <ul id="ccm-search-block-tag-cloud-<?php echo $bID ?>" class="ccm-search-block-tag-cloud">
        ...
        for ($i = 0; $i < $ttags->count(); $i++) {
            $akct = $tags[$i];
            $qs = urlencode($akc->field('atSelectOptionID') . '[]') . '='
                  . urlencode($akct->getSelectAttributeOptionID());
            ?>
            <li><a style="font-size: <?php echo $tagSizes[$akct->getSelectAttributeOptionUsageCount()] ?>px !important"
                   href="<?php echo $view->url($resultTargetURL) ?>?<?php echo $qs ?>"><?php echo $akct->getSelectAttributeOptionValue() ?></a>
