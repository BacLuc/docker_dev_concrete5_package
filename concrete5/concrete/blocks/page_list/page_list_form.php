...
<div class="row pagelist-form">
    <div class="col-xs-6">
        ...
        <input type="hidden" name="pageListToolsDir"
               value="<?php echo Loader::helper('concrete/urls')->getBlockTypeToolsURL($bt) ?>/"/>
                ...
                if (is_array($ctArray)) {
                    ...
                    <select class="form-control" name="ptID" id="selectPTID">
                        ...
                        foreach ($ctArray as $ct) {
                            ?>
                            <option
                                    value="<?php echo $ct->getPageTypeID() ?>" <?php if ($ptID
                                                                                         == $ct->getPageTypeID()) { ?> selected <?php } ?>>
        ...
        <fieldset>
                    ...
                    <select class="form-control" name="customTopicAttributeKeyHandle"
                        ...
                        <?php foreach ($attributeKeys as $attributeKey) {
                            ...
                            <option data-topic-tree-id="<?php echo $attributeController->getTopicTreeID() ?>"
                                    ...
                                    <?php if ($attributeKey->getAttributeKeyHandle()
                                              == $customTopicAttributeKeyHandle) { ?>selected<?php } ?>><?php echo $attributeKey->getAttributeKeyDisplayName() ?></option>
                        <?php } ?>
                    </select>
                </div>
            ...
            <div data-row="related-topic">
                <div class="form-group">
                    ...
                    <select class="form-control" name="relatedTopicAttributeKeyHandle"
                            id="relatedTopicAttributeKeyHandle">
                        <option value=""><?php echo t('Choose topics attribute.') ?></option>
                        <?php foreach ($attributeKeys as $attributeKey) { ?>
                            <option value="<?php echo $attributeKey->getAttributeKeyHandle() ?>"
                                    <?php if ($attributeKey->getAttributeKeyHandle()
                                              == $relatedTopicAttributeKeyHandle) { ?>selected<?php } ?>><?php echo $attributeKey->getAttributeKeyDisplayName() ?></option>
                        <?php } ?>
                    </select>
                </div>
        </fieldset>
        ...
        <fieldset>
            ...
            <div class="form-group">
                ...
                foreach ($filterDateOptions as $filterDateOptionHandle => $filterDateOptionLabel) {
                    ...
                    <div class="radio">
                        <label>
                            <input type="radio" class='filterDateOption' name="filterDateOption"
                                   value="<?php echo $filterDateOptionHandle ?>" <?php echo $isChecked ?> />
                ...
                <div class="filterDateOptionDetail" data-filterDateOption="past">
                    <div class="form-group">
                        <label class="control-label"><?php echo t('Days in the Past') ?> <i
                                    class="launch-tooltip fa fa-question-circle"
                                    title="<?php echo t('Leave 0 to show all past dated pages') ?>"></i></label>
                        <input type="text" name="filterDatePast" value="<?php echo $filterDateDays ?>"
                               class="form-control">
                    </div>
                </div>
                ...
                <div class="filterDateOptionDetail" data-filterDateOption="future">
                    <div class="form-group">
                        <label class="control-label"><?php echo t('Days in the Future') ?> <i
                                    class="launch-tooltip fa fa-question-circle"
                                    title="<?php echo t('Leave 0 to show all future dated pages') ?>"></i></label>
                        <input type="text" name="filterDateFuture" value="<?php echo $filterDateDays ?>"
                               class="form-control">
                    </div>
                </div>
        ...
        <fieldset>
            ...
            <div class="checkbox">
                <label>
                    <input <?php if (!is_object($featuredAttribute)) { ?> disabled <?php } ?> type="checkbox"
                                                                                              name="displayFeaturedOnly"
                                                                                              value="1" <?php if ($displayFeaturedOnly
                                                                                                                  == 1) { ?> checked <?php } ?>
            ...
            <div class="checkbox">
                <label>
                    <input type="checkbox" name="enableExternalFiltering" value="1"
                           <?php if ($enableExternalFiltering) { ?>checked<?php } ?> />
        ...
        <fieldset>
                ...
                <div class="radio">
                    <label>
                        <input type="radio" name="rss" class="rssSelector"
                               value="0" <?php echo(is_object($rssFeed) ? "" :
                            "checked=\"checked\"") ?>/> <?php echo t('No') ?>
                    </label>
                </div>
                <div class="radio">
                    <label>
                        <input id="ccm-pagelist-rssSelectorOn" type="radio" name="rss" class="rssSelector"
                               value="1" <?php echo(is_object($rssFeed) ? "checked=\"checked\"" :
                            "") ?>/> <?php echo t('Yes') ?>
                    </label>
                </div>
                <div id="ccm-pagelist-rssDetails" <?php echo(is_object($rssFeed) ? "" : "style=\"display:none;\"") ?>>
                    <?php if (is_object($rssFeed)) { ?>
                        <?php echo t('RSS Feed can be found here: <a href="%s" target="_blank">%s</a>',
                            $rssFeed->getFeedURL(), $rssFeed->getFeedURL()) ?>
                    <?php } else { ?>
                        <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="radio">
                    <label>
                        <input type="radio" name="includeName"
                               value="1" <?php echo($includeName ? "checked=\"checked\"" :
                            "") ?>/> <?php echo t('Yes') ?>
                    </label>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="radio">
                    <label>
                        <input type="radio" name="includeDescription"
                               value="0" <?php echo($includeDescription ? "" :
                            "checked=\"checked\"") ?>/> <?php echo t('No') ?>
                    </label>
                </div>
                <div class="radio">
                    <label>
                        <input type="radio" name="includeDescription"
                               value="1" <?php echo($includeDescription ? "checked=\"checked\"" :
                            "") ?>/> <?php echo t('Yes') ?>
                    </label>
                </div>
                <div class="ccm-page-list-truncate-description" <?php echo($includeDescription ? "" :
                    "style=\"display:none;\"") ?>>
                ...
                <span class="input-group-addon">
                    <input id="ccm-pagelist-truncateSummariesOn" name="truncateSummaries" type="checkbox"
                        ...
                        <input class="form-control" id="ccm-pagelist-truncateChars" <?php echo($truncateSummaries ? "" :
                            "disabled=\"disabled\"") ?>
            ...
            <div class="form-group">
                ...
                <div class="radio">
                    <label>
                        <input type="radio" name="includeDate"
                               value="1" <?php echo($includeDate ? "checked=\"checked\"" :
                            "") ?>/> <?php echo t('Yes') ?>
                    </label>
                </div>
                    ...
                    <label>
                        <input type="radio" name="displayThumbnail"
                            <?php echo(!is_object($thumbnailAttribute) ? 'disabled ' : '') ?>
                               value="0" <?php echo($displayThumbnail ? "" :
                            "checked=\"checked\"") ?>/> <?php echo t('No') ?>
                    </label>
                </div>
                <div class="radio">
                    <label>
                        <input type="radio" name="displayThumbnail"
                            <?php echo(!is_object($thumbnailAttribute) ? 'disabled ' : '') ?>
                               value="1" <?php echo($displayThumbnail ? "checked=\"checked\"" :
                            "") ?>/> <?php echo t('Yes') ?>
                    </label>
                </div>
                ...
                <div class="radio">
                    <label>
                        <input type="radio" name="useButtonForLink"
                               value="0" <?php echo($useButtonForLink ? "" :
                            "checked=\"checked\"") ?>/> <?php echo t('No') ?>
                    </label>
                </div>
                <div class="radio">
                    <label>
                        <input type="radio" name="useButtonForLink"
                               value="1" <?php echo($useButtonForLink ? "checked=\"checked\"" :
                            "") ?>/> <?php echo t('Yes') ?>
                    </label>
                </div>
                <div class="ccm-page-list-button-text" <?php echo($useButtonForLink ? "" :
                    "style=\"display:none;\"") ?>>
                    <div class="form-group">
                        <label class="control-label"><?php echo t('Link Text') ?></label>
                        <input class="form-control" type="text" name="buttonLinkText"
                               value="<?php echo $buttonLinkText ?>"/>
                    </div>
                </div>
            </div>
