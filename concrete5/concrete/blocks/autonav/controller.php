...
class Controller extends BlockController
{
    ...
    {
        $args['displayPagesIncludeSelf'] =
            isset($args['displayPagesIncludeSelf']) && $args['displayPagesIncludeSelf'] ? 1 : 0;
        $args['displayPagesCID'] =
            isset($args['displayPagesCID']) && $args['displayPagesCID'] ? $args['displayPagesCID'] : 0;
        $args['displaySubPageLevelsNum'] =
            isset($args['displaySubPageLevelsNum']) && $args['displaySubPageLevelsNum'] > 0 ?
                $args['displaySubPageLevelsNum'] : 0;
        $args['displayUnavailablePages'] =
            isset($args['displayUnavailablePages']) && $args['displayUnavailablePages'] ? 1 : 0;
    ...
    public function getNavItems($ignore_exclude_nav = false)
    {
        ...
        $excluded_parent_level =
            9999; //Arbitrarily high number denotes that we're NOT currently excluding a parent (because all actual page levels will be lower than this)
        $exclude_children_below_level =
            9999; //Same deal as above. Note that in this case "below" means a HIGHER number (because a lower number indicates higher placement in the sitemp -- e.g. 0 is top-level)
        foreach ($allNavItems as $ni) {
            ...
            if ($this->excludeFromNavViaAttribute($_c) && ($current_level <= $excluded_parent_level)) {
                ...
                if (($current_level > $excluded_parent_level) || ($current_level > $exclude_children_below_level)) {
                    ...
                    $excluded_parent_level =
                        9999; //Reset to arbitrarily high number to denote that we're no longer excluding a parent
        ...
        for ($i = 0; $i < $navItemCount; ++$i) {
            ...
            if ($_c->getAttribute('replace_link_with_first_in_nav')) {
                $subPage =
                    $_c->getFirstChild(); //Note: could be a rare bug here if first child was excluded, but this is so unlikely (and can be solved by moving it in the sitemap) that it's not worth the trouble to check
    ...
    public function generateNav()
    {
        ...
        if (isset($this->displayPagesCID)
            && !Core::make('helper/validation/numbers')->integer($this->displayPagesCID)) {
    ...
    public function getNavigationArray($cParentID, $orderBy, $currentLevel)
    {
        ...
        foreach ($this->navArray as $ni) {
            if ($ni->getCollectionID() == $cParentID && $this->ignoreExcludeNav === false) {
                if ($ni->getCollectionObject()->getAttribute('exclude_nav') == 1
                    || $ni->getCollectionObject()->getAttribute('exclude_subpages_from_nav') == 1) {
                    return;
                }
            }
        }
        ...
        $q =
            "select Pages.cID from Pages where cIsTemplate = 0 and cIsActive = 1 and cParentID = '{$cParentID}' {$orderBy}";
        $r = $db->query($q);
        if ($r) {
            while ($row = $r->fetchRow()) {
                if ($this->displaySubPages != 'relevant_breadcrumb'
                    || (in_array(
                            $row['cID'],
                            $this->cParentIDArray)
                        || $row['cID'] == $this->cID)
                ...
                if ($sortit) {
                    foreach ($navSort as $sortCID => $sortdatewhocares) {
                        ...
                        if ($this->displaySubPages == 'all') {
                            if ($this->displaySubPageLevels == 'all'
                                || ($this->displaySubPageLevels == 'custom'
                                    && $this->displaySubPageLevelsNum > $currentLevel)) {
                                $retrieveMore = true;
                            }
                        } else {
                            if (($this->displaySubPages == "relevant"
                                 || $this->displaySubPages == "relevant_breadcrumb")
                                && (in_array(
                                        $sortCID,
                                        $this->cParentIDArray)
                                    || $sortCID == $this->cID)
                                ...
                                if ($this->displaySubPageLevels == "enough" && $this->haveRetrievedSelf == false) {
                                    ...
                                    if ($this->displaySubPageLevels == "enough_plus1"
                                        && $this->haveRetrievedSelfPlus1 == false) {
                                        $retrieveMore = true;
                                    } else {
                                        if ($this->displaySubPageLevels == 'all'
                                            || ($this->displaySubPageLevels == 'custom'
