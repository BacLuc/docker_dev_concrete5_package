...
class Controller extends BlockController
{
                    ...
                    if (isset($data->record)) {
                        foreach ($data->record as $record) {
                            ...
                            if ($table == 'btFormQuestions') {
                                ...
                                $aar->questionSetId = $db->GetOne('SELECT questionSetId FROM btForm WHERE bID = ?',
                                    array($b->getBlockID()));
                            }
                            $aar->Replace();
                        }
    ...
    public function save($data = array())
    {
        ...
        if (intval($total) == 0) {
            $q =
                "insert into {$this->btTable} (questionSetId, surveyName, submitText, notifyMeOnSubmission, recipientEmail, thankyouMsg, displayCaptcha, redirectCID, addFilesToSet, bID) values (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
        } else {
            $v[] = $data['qsID'];
            $q =
                "update {$this->btTable} set questionSetId = ?, surveyName=?, submitText=?, notifyMeOnSubmission=?, recipientEmail=?, thankyouMsg=?, displayCaptcha=?, redirectCID=?, addFilesToSet=? where bID = ? AND questionSetId= ?";
        }
    ...
    protected function questionVersioning($data = array())
    {
        ...
        $unchangedQuestions =
            $db->query('DELETE FROM btFormQuestions WHERE bID=? AND questionSetId=? AND msqID IN (' . $pendingDeleteQIDs
                       . ')', $vals);
    }
    ...
    public function duplicate($newBID)
    {
        ...
        if (count($row) > 0) {
            ...
            $questionSetCIDs =
                $db->getCol("SELECT distinct cID FROM {$this->btTable} AS f, CollectionVersionBlocks AS cvb " .
            ...
            $q =
                "insert into {$this->btTable} ( questionSetId, surveyName, submitText, bID,thankyouMsg,notifyMeOnSubmission,recipientEmail,displayCaptcha,addFilesToSet) values (?, ?, ?, ?, ?, ?, ?, ?,?)";
            $result = $db->Execute($q, $v);

            $rs =
                $db->query("SELECT * FROM {$this->btQuestionsTablename} WHERE questionSetId=$oldQuestionSetId AND bID="
                           . intval($this->bID));
            while ($row = $rs->fetchRow()) {
                ...
                $sql =
                    "INSERT INTO {$this->btQuestionsTablename} (questionSetId,msqID,bID,question,inputType,options,position,width,height,required,defaultDate) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
                $db->Execute($sql, $v);
            }
    ...
    public function action_submit_form($bID = false)
    {
        ...
        $rows =
            $db->GetArray("SELECT * FROM {$this->btQuestionsTablename} WHERE questionSetId=? AND bID=? order by position asc, msqID",
                array($qsID, intval($this->bID)));
        ...
        foreach ($rows as $row) {
            ...
            if (intval($row['required']) == 1) {
                ...
                if ($row['inputType'] == 'checkboxlist') {
                    ...
                    if (!isset($_FILES['Question' . $row['msqID']])
                        || !is_uploaded_file($_FILES['Question' . $row['msqID']]['tmp_name'])) {
                        $notCompleted = 1;
                    }
        ...
        if (!count($errors)) {
            foreach ($rows as $row) {
                ...
                if (!intval($row['required'])
                    && (
                        !isset($_FILES[$questionName]['tmp_name'])
                        || !is_uploaded_file($_FILES[$questionName]['tmp_name'])
                    )
                ) {
                    continue;
                }
        ...
        if (count($errors)) {
            ...
            foreach ($rows as $row) {
                ...
                if ($row['inputType'] == 'checkboxlist') {
                    ...
                    if (!empty($row['options'])) {
                        ...
                        if (is_array($settings) && array_key_exists('send_notification_from', $settings)
                            && $settings['send_notification_from'] == 1) {
                ...
                $questionAnswerPairs[$row['msqID']]['answerDisplay'] =
                    strlen($answerDisplay) ? $answerDisplay : $questionAnswerPairs[$row['msqID']]['answer'];
            ...
            if (!$this->noSubmitFormRedirect) {
                ...
                if (is_object($targetPage)) {
                    ...
                    $url = $response->getTargetUrl() . "?surveySuccess=1&qsid=" . $this->questionSetId . "#formblock"
                           . $this->bID;
    ...
    public function delete()
    {
        ...
        $deleteData['questionsIDs'] = $db->getAll("SELECT qID FROM {$this->btQuestionsTablename} WHERE questionSetId = "
                                                  . intval($info['questionSetId']) . ' AND bID=' . intval($this->bID));
        ...
        $strandedAnswerIDs =
            $db->getAll('SELECT fa.aID FROM `btFormAnswers` AS fa LEFT JOIN btFormQuestions AS fq ON fq.msqID=fa.msqID WHERE fq.msqID IS NULL');
        ...
        $deleteData['strandedAnswerSetIDs'] =
            $db->getAll('SELECT aset.asID FROM btFormAnswerSet AS aset LEFT JOIN btFormAnswers AS fa ON aset.asID=fa.asID WHERE fa.asID IS NULL');
