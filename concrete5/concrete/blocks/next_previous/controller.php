    ...
    public function getNextCollection()
    {
        ...
        while ($cID > 0) {
            switch ($this->orderBy) {
                case 'chrono_desc':
                    $cID =
                        $db->GetOne('SELECT Pages.cID FROM Pages INNER JOIN CollectionVersions cv ON Pages.cID = cv.cID WHERE cvIsApproved = 1 AND cvDatePublic > ? AND cParentID = ? '
                                    . $systemPages . ' ORDER BY cvDatePublic ASC',
                            array($currentPage->getCollectionDatePublic(), $currentPage->getCollectionParentID()));
                    break;
                case 'chrono_asc':
                    $cID =
                        $db->GetOne('SELECT Pages.cID FROM Pages INNER JOIN CollectionVersions cv ON Pages.cID = cv.cID WHERE cvIsApproved = 1 AND cvDatePublic < ? AND cParentID = ? '
                                    . $systemPages . ' ORDER BY cvDatePublic DESC',
                            array($currentPage->getCollectionDatePublic(), $currentPage->getCollectionParentID()));
                    break;
                case 'display_desc':
                    $cID = $db->GetOne('SELECT cID FROM Pages WHERE cDisplayOrder < ? AND cParentID = ? ' . $systemPages
                                       . ' ORDER BY cDisplayOrder DESC',
                        array($currentPage->getCollectionDisplayOrder(), $currentPage->getCollectionParentID()));
                    break;
                case 'display_asc':
                default:
                    $cID = $db->GetOne('SELECT cID FROM Pages WHERE cDisplayOrder > ? AND cParentID = ? ' . $systemPages
