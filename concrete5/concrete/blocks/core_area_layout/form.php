...
<ul id="ccm-layouts-toolbar" class="ccm-inline-toolbar ccm-ui">
            ...
            <?php if (count($presets) > 0) { ?>
                <optgroup label="<?php echo t('Presets') ?>">
                    <?php foreach ($presets as $pr) { ?>
                        <option value="<?php echo $pr->getIdentifier() ?>" <?php if (is_object($selectedPreset)
                                                                                     && $selectedPreset->getIdentifier()
                                                                                        == $pr->getIdentifier()) { ?>selected<?php } ?>><?php echo $pr->getName() ?></option>
                    <?php } ?>
                </optgroup>
            <?php } ?>
        </select>
    ...
    <li data-grid-form-view="themegrid">
        ...
        <input type="text" name="themeGridColumns" id="themeGridColumns"
               style="width: 40px" <?php if ($controller->getTask()
                                             == 'add') { ?>  data-input="number" data-minimum="<?php echo $minColumns ?>" data-maximum="<?php echo $themeGridMaxColumns ?>" <?php } ?>
    ...
    <li data-grid-form-view="custom" class="ccm-sub-toolbar-text-cell">
        ...
        <input type="text" name="columns" id="columns" style="width: 40px" <?php if ($controller->getTask()
                                                                                     == 'add') { ?> data-input="number" data-minimum="<?php echo $minColumns ?>" data-maximum="<?php echo $maxColumns ?>" <?php } ?>
               value="<?php echo $columnsNum ?>"/>
        <?php if ($controller->getTask() == 'edit') {
            // we need this to actually go through the form in edit mode, for layout presets to be saveable in edit mode. ?>
    ...
    <li data-grid-form-view="custom">
        ...
        <input name="spacing" id="spacing" type="text" style="width: 40px" data-input="number" data-minimum="0"
               data-maximum="1000" value="<?php echo $spacing ?>"/>
    </li>
    <li data-grid-form-view="custom"
        class="ccm-inline-toolbar-icon-cell <?php if (!$iscustom) { ?>ccm-inline-toolbar-icon-selected<?php } ?>"><a
                href="#" data-layout-button="toggleautomated"><i class="fa fa-lock"></i></a>
        <input type="hidden" name="isautomated" value="<?php if ($iscustom) { ?>0<?php } else { ?>1<?php } ?>"/>
    </li>
    <?php if ($controller->getTask() == 'edit') {
        $bp = new Permissions($b); ?>

        <li class="ccm-inline-toolbar-icon-cell"><a href="#" data-layout-command="move-block"><i
        ...
        if ($bp->canDeleteBlock()) {
            ...
            ?>
            <li class="ccm-inline-toolbar-icon-cell"><a href="#" data-menu-action="delete-layout"><i
    ...
    <li class="ccm-inline-toolbar-button ccm-inline-toolbar-button-save">
        <button class="btn btn-primary" type="button" id="ccm-layouts-save-button"><?php if ($controller->getTask()
                                                                                             == 'add') { ?><?php echo t('Add Layout') ?><?php } else { ?><?php echo t('Update Layout') ?><?php } ?></button>
    </li>
</ul>

<?php if ($controller->getTask() == 'add') { ?>
    <input name="arLayoutMaxColumns" type="hidden"
           value="<?php echo $view->getAreaObject()->getAreaGridMaximumColumns() ?>"/>
<?php } ?>
    ...
    $(function () {
        ...
        $('#ccm-layouts-toolbar').on('click', 'a[data-menu-action=delete-layout]', function (e) {
            ...
            Concrete.event.fire('EditModeBlockDelete', {
                message: '<?php echo $deleteMessage?>',
                block: block,
                event: e
            });
            return false;
...
<div class="ccm-area-layout-control-bar-wrapper">
    <div id="ccm-area-layout-active-control-bar"
         class="ccm-area-layout-control-bar ccm-area-layout-control-bar-<?php echo $controller->getTask() ?>"></div>
</div>
