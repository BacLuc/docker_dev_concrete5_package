            ...
            <div data-form="discussion">
                <form data-form="composer">
                    ...
                    <div class="dialog-buttons">
                        <button type="button" data-composer-btn="exit"
                                class="btn btn-default pull-left"><?php echo t('Cancel') ?></button>
                        <button type="button" data-composer-btn="publish"
                                class="btn btn-primary pull-right"><?php echo t('Post') ?></button>
                    </div>
                </form>
            </div>
        </div>

        <button class="pull-right btn" data-action="add-conversation"
                type="button"><?php echo t('New Topic') ?></button>

    <?php } ?>

    <?php if ($enableOrdering) { ?>
        <select name="orderBy" class="ccm-discussion-order-by" data-select="order">
            <option data-sort-url="<?php echo Loader::helper('url')->setVariable('orderBy', 'date_last_message') ?>"
                    value="date_last_message" <?php if ($reqOrderBy
                                                        == 'date_last_message') { ?>selected<?php } ?>><?php echo t('Recent Comment') ?></option>
            <option data-sort-url="<?php echo Loader::helper('url')->setVariable('orderBy', 'date') ?>" value="date"
                    <?php if ($reqOrderBy == 'date') { ?>selected<?php } ?>><?php echo t('Original Post') ?></option>
            <option data-sort-url="<?php echo Loader::helper('url')->setVariable('orderBy', 'replies') ?>"
                    value="replies"
                    <?php if ($reqOrderBy == 'replies') { ?>selected<?php } ?>><?php echo t('Activity') ?></option>
        </select>
    <?php } ?>
            ...
            <?php foreach ($topics as $t) {
                ...
                <li>
                    ...
                    <div class="ccm-discussion-topic-details">
                        <h3><a href="<?php echo Loader::helper('navigation')
                                                      ->getLinkToCollection($t) ?>"><?php echo $t->getCollectionName() ?></a>
                        </h3>
                        <p><?php echo t('Topic Posted on %s.',
                                $dh->formatDateTime($t->getCollectionDatePublic(), true)) ?>
                            <?php if ($replies > 0) { ?>
                                <?php echo t('Last Message Posted on %s.',
                                    $dh->formatDateTime($cnv->getConversationDateLastMessage(), true)) ?>
                            <?php } ?>
                        </p>
                    </div>
                </li>
