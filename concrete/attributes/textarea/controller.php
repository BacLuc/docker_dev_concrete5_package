...
{
    protected $searchIndexFieldDefinition = array('type'    => 'text',
    ...
    public function form($additionalClass = false)
    {
        ...
        if ($this->akTextareaDisplayMode == 'text' || $this->akTextareaDisplayMode == '') {
            print Core::make('helper/form')
                      ->textarea($this->field('value'), $value, array('class' => $additionalClass, 'rows' => 5))
            ;
        } else {
    ...
    */
    ...
    protected function load()
    {
        ...
        $db = Database::connection();
        $row =
            $db->GetRow('SELECT akTextareaDisplayMode, akTextareaDisplayModeCustomOptions FROM atTextareaSettings WHERE akID = ?',
