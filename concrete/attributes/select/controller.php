...
class Controller extends AttributeTypeController
{
    ...
    protected function load()
    {
        ...
        $db = Database::get();
        $row =
            $db->GetRow('SELECT akSelectAllowMultipleValues, akSelectOptionDisplayOrder, akSelectAllowOtherValues FROM atSelectSettings WHERE akID = ?',
    ...
    public function exportKey($akey)
    {
        ...
        $r =
            $db->Execute('SELECT value, displayOrder, isEndUserAdded FROM atSelectOptions WHERE akID = ? ORDER BY displayOrder ASC',
                $this->getAttributeKey()->getAttributeKeyID());
    ...
    public function getSelectedOptions()
    {
        ...
        switch ($this->akSelectOptionDisplayOrder) {
            case 'popularity_desc':
                $options =
                    $db->GetAll("SELECT ID, value, displayOrder, (SELECT count(s2.atSelectOptionID) FROM atSelectOptionsSelected s2 WHERE s2.atSelectOptionID = ID) AS total FROM atSelectOptionsSelected INNER JOIN atSelectOptions ON atSelectOptionsSelected.atSelectOptionID = atSelectOptions.ID WHERE avID = ? ORDER BY total DESC, value ASC",
                        array($this->getAttributeValueID()));
                break;
            case 'alpha_asc':
                $options =
                    $db->GetAll("SELECT ID, value, displayOrder FROM atSelectOptionsSelected INNER JOIN atSelectOptions ON atSelectOptionsSelected.atSelectOptionID = atSelectOptions.ID WHERE avID = ?",
                        array($this->getAttributeValueID()));
                $sortByDisplayName = true;
                break;
            default:
                $options =
                    $db->GetAll("SELECT ID, value, displayOrder FROM atSelectOptionsSelected INNER JOIN atSelectOptions ON atSelectOptionsSelected.atSelectOptionID = atSelectOptions.ID WHERE avID = ? ORDER BY displayOrder ASC",
                        array($this->getAttributeValueID()));
                break;
        }
        $db = Database::get();
    ...
    public function getOptions($like = null)
    {
        ...
        switch ($this->akSelectOptionDisplayOrder) {
            ...
            case 'alpha_asc':
                if (isset($like) && strlen($like)) {
                    $r =
                        $db->Execute('SELECT ID, value, displayOrder FROM atSelectOptions WHERE akID = ? AND atSelectOptions.value LIKE ? ORDER BY value ASC',
                            array($this->attributeKey->getAttributeKeyID(), $like));
                } else {
                    $r =
                        $db->Execute('SELECT ID, value, displayOrder FROM atSelectOptions WHERE akID = ? ORDER BY value ASC',
                            array($this->attributeKey->getAttributeKeyID()));
                }
                break;
            default:
                if (isset($like) && strlen($like)) {
                    $r =
                        $db->Execute('SELECT ID, value, displayOrder FROM atSelectOptions WHERE akID = ? AND atSelectOptions.value LIKE ? ORDER BY displayOrder ASC',
                            array($this->attributeKey->getAttributeKeyID(), $like));
                } else {
                    $r =
                        $db->Execute('SELECT ID, value, displayOrder FROM atSelectOptions WHERE akID = ? ORDER BY displayOrder ASC',
                            array($this->attributeKey->getAttributeKeyID()));
                }
                break;
        }
    ...
    public function saveKey($data)
    {
        ...
        if (isset($data['akSelectOptionDisplayOrder'])
            && in_array($data['akSelectOptionDisplayOrder'],
