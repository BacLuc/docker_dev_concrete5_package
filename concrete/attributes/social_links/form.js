var ConcreteSocialLinksAttribute = {
    ...
    init: function () {
        $('.ccm-social-link-attribute-add-service').click(function () {
            $($('.ccm-social-link-attribute').get(0)).clone().appendTo($('.ccm-social-link-attribute-wrapper')).find('input').val('').closest('.ccm-social-link-attribute').find('select').trigger('change');
        ...
        $('.ccm-social-link-attribute-wrapper').on('change', 'select', function () {
            ...
            $parentContainer.find('.ccm-social-link-service-text-wrapper').addClass('input-prepend').find('.ccm-social-link-service-add-on-wrapper').show().find('.add-on i').removeClass().addClass('fa fa-' + opt.attr('data-icon'));
